...
constexpr std::array<std::string_view, 15> standard_iwads = {
...
void D_PostEvent(event_t& ev) {
  ...
  M_Responder(&ev) || (gamestate == GS_LEVEL && (HU_Responder(&ev) || ST_Responder(&ev) || AM_Responder(&ev)))
      || G_Responder(&ev);
}

void D_PostEvent(event_t* const ev) {
  D_PostEvent(*ev);
}
...
void D_Wipe() {
  ...
  do {
    ...
    if (capturing_video && !doSkip && cap_wipescreen != 0) {
      I_CaptureFrame();
    }
  } while (!done);
}
...
extern "C" bool setsizeneeded;
extern int showMessages;

void D_Display(const fixed_t frac) {
  ...
  if (gamestate != GS_LEVEL) {  // Not a level
    ...
    if (oldgamestate != GS_LEVEL) {
      ...
      borderwillneedredraw = menuactive != mnact_inactive && isborder && viewactive;
      // e6y
      // I should do it because I call R_RenderPlayerView in all cases,
      // not only if viewactive is true
      borderwillneedredraw =
          (borderwillneedredraw) || ((((automapmode & am_active) != 0) && ((automapmode & am_overlay) == 0)));
    }
    ...
    // Now do the drawing
    if (viewactive || map_always_updates != 0) {
    ...
    ST_Drawer(((viewheight != SCREENHEIGHT) || (((automapmode & am_active) != 0) && !(automapmode & am_overlay))),
              redrawborderstuff || BorderNeedRefresh, (menuactive == mnact_full));
  ...
  // draw pause pic
  if (paused != 0 && menuactive != mnact_full) {
  ...
  if ((paused != 0 && !walkcamera.type) || !window_focused) {
    I_uSleep(5000);
  }
...
void D_AddFile(const char* file, wad_source_t source) {
  ...
  wadfiles[numwadfiles].name = AddDefaultExtension(strcpy(static_cast<char*>(malloc(strlen(file) + 5)), file), ".wad");
  ...
  gwa_filename = AddDefaultExtension(strcpy(static_cast<char*>(malloc(strlen(file) + 5)), file), ".wad");
...
// e6y static
extern "C" const char* D_dehout(void) {
  ...
  if (!M_access(iwadname, R_OK)) {
    ...
    if ((fp = M_fopen(iwadname, "rb"))) {
      ...
      if (fread(&header, sizeof(header), 1, fp) == 1) {
        ...
        fileinfo = static_cast<filelump_t*>(malloc(length * sizeof(filelump_t)));
        if (fseek(fp, header.infotableofs, SEEK_SET) || fread(fileinfo, sizeof(filelump_t), length, fp) != length) {
          fclose(fp);
...
static char* FindIWADFile(void) {
  ...
  if (i && (++i < myargc)) {
    ...
    for (i = 0; iwad == nullptr && i < standard_iwads.size(); i++)
      iwad = I_FindFile(standard_iwads[i].data(), ".wad");
  }
  return iwad;
}
...
static void FindResponseFile(void) {
  ...
  for (i = 1; i < myargc; i++)
    if (myargv[i][0] == '@') {
      ...
      int indexinfile;
      byte* file = NULL;
      const char** moreargs = static_cast<const char**>(malloc(myargc * sizeof(const char*)));
      char** newargv;
      ...
      // proff 04/05/2000: Added for searching responsefile
      if (size < 0) {
        size_t fnlen = doom_snprintf(NULL, 0, "%s/%s", I_DoomExeDir(), &myargv[i][1]);
        fname = static_cast<char*>(realloc(fname, fnlen + 4 + 1));
        doom_snprintf(fname, fnlen + 1, "%s/%s", I_DoomExeDir(), &myargv[i][1]);
        AddDefaultExtension(fname, ".rsp");
        size = M_ReadFile(fname, &file);
      }
      if (size < 0) {
      ...
      if (size <= 0) {
        ...
        newargv = static_cast<char**>(calloc(sizeof(newargv[0]), myargc));
        newargv[0] = myargv[0];
...
static void DoLooseFiles(void) {
  ...
  struct {
    ...
    char*** list;
    int* count;
  } params[] = {{"-file", &wads, &wadcount}, {"-deh", &dehs, &dehcount}, {"-playdemo", &lmps, &lmpcount}, {0}};
...
static char* GetAutoloadBaseDir(unsigned int iter) {
  ...
  if (autoload_path == NULL) {
    ...
    autoload_path = static_cast<char*>(malloc(len + 1));
...
const char* BaseName(const char* filename) {
  ...
  basename = static_cast<const char*>(filename) + strlen(filename) - 1;
...
static void D_DoomMainSetup(void) {
  ...
  */
...

  {
    ...
    lprintf(LO_ALWAYS,
            PACKAGE_NAME " (built %s), playing: %s\n" PACKAGE_NAME
  ...
  if (!M_CheckParm("-nodeh")) {
    ...
    for (p = -1; (p = W_ListNumFromName("DEHACKED", p)) >= 0;)
      ...
      if (lumpinfo[p].source == source_iwad || lumpinfo[p].source == source_pre
  ...
  if (p) {
    ...
    while (++p != myargc && *myargv[p] != '-') {
      char* file = NULL;
      if ((file = I_FindFile(myargv[p], ".bex")) || (file = I_FindFile(myargv[p], ".deh"))) {
  ...
  if (*map == -1)    // unknown so go search for first changed one
  {
    ...
    *map = 1;  // default E1M1 or MAP01
    ...
    lprintf(LO_CONFIRM, "Auto-warping to first %slevel: %s\n", newlevel ? "new " : "",
            name);  // Ty 10/04/98 - new level test
  }
}
