...
/*
   ...
   // TODO(Derppening): Refactor to C++20
...
static const Bit8u AttackSamplesTable[13] = {69, 55, 46, 40, 35, 29, 23, 20, 19, 15, 11, 10, 9};
...
static const Bit16u WaveBaseTable[8] = {
    0x000, 0x200, 0x200, 0x800, 0xa00, 0xc00, 0x100, 0x400,

};
// Mask the counter with this
static const Bit16u WaveMaskTable[8] = {
    1023, 1023, 511, 511, 1023, 1023, 512, 1023,
};

// Where to start the counter on at keyon
static const Bit16u WaveStartTable[8] = {
    512, 0, 0, 0, 0, 512, 512, 256,
};
#endif
...
static const Bit8s VibratoTable[8] = {1 - 0x00, 0 - 0x00, 1 - 0x00, 30 - 0x00, 1 - 0x80, 0 - 0x80, 1 - 0x80, 30 - 0x80};
...
static const VolumeHandler VolumeHandlerTable[5] = {
    &Operator::TemplateVolume<Operator::OFF>, &Operator::TemplateVolume<Operator::RELEASE>,
    &Operator::TemplateVolume<Operator::SUSTAIN>, &Operator::TemplateVolume<Operator::DECAY>,
...
void InitTables(void) {
  ...
  for (Bitu i = 0; i < 32; i++) {
    ...
    if (i >= 16)
      index += 9;
    ChanOffsetTable[i] = 1 + (Bit16u)(index * sizeof(Channel));
  }
  // Same for operators
  for (Bitu i = 0; i < 64; i++) {
    if (i % 8 >= 6 || ((i / 8) % 4 == 3)) {
    ...
    if (chNum >= 12)
      chNum += 16 - 12;
    Bitu opNum = (i % 8) / 3;
    OpOffsetTable[i] = ChanOffsetTable[chNum] + (Bit16u)(opNum * sizeof(Operator));
  }
#if 0
   DBOPL::Chip* chip = 0;
...
void Chip__Chip(Chip** const self) {
  *self = new Chip{false};
}
...
void Chip__GenerateBlock2(Chip* self, Bitu total, Bit32s* output) {
  self->GenerateBlock2(total, output);
}
